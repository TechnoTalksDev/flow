	...
	<Card.Root class="w-[400px] max-w-[98vw]">
      ...
      {:then userData}
        <div class="space-y-4">
          ...
          <div>
            <h3 class="text-sm font-medium text-muted-foreground mb-2">Account Stats</h3>
          ...
          <div>
            <h3 class="text-sm font-medium text-muted-foreground mb-2">Account Details</h3>
            <div class="rounded-md bg-muted p-4 text-sm">
              <p><span class="font-semibold">User ID:</span> {userData.id}</p>
              <p><span class="font-semibold">Joined:</span> {new Date(userData.created_at).toLocaleDateString()}</p>
            </div>
          </div>
        </div>
      {:catch error}
      ...
      {#await data.userData}
        <Skeleton class="h-4 w-full" />
      {:then userData}
        <a href=""><p class="text-sm text-muted-foreground text-center flex flex-row">Share <ArrowRight size="20px" /> </p></a>
      {:catch}
